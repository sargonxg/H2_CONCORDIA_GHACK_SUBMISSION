...
export function formatAgreementAsText(
  agreement: {
    preamble?: string;
    background?: string;
  ...
  if (agreement.preamble) {
    lines.push(agreement.preamble, '');
  }

  if (agreement.background) {
    lines.push('BACKGROUND', agreement.background, '');
  }

  if (agreement.agreedTerms?.length) {
    lines.push('AGREED TERMS');
    agreement.agreedTerms.forEach((t) => {
      lines.push(
        `${t.number}. ${t.title}\n   ${t.text}\n   Responsible: ${t.responsible || 'N/A'} | Deadline: ${t.deadline || 'N/A'}`,
      );
    });
    lines.push('');
  }

  if (agreement.implementationPlan) {
    lines.push('IMPLEMENTATION PLAN', agreement.implementationPlan, '');
  }

  if (agreement.reviewMechanism) {
    lines.push('REVIEW MECHANISM', agreement.reviewMechanism, '');
  }

  if (agreement.confidentiality) {
    lines.push('CONFIDENTIALITY', agreement.confidentiality, '');
  }
  ...
  if (agreement.disclaimer) {
    lines.push(agreement.disclaimer, '');
  }

  lines.push('Generated by CONCORDIA — TACITUS Institute for Conflict Resolution — tacitus.me');
